  require("crypto-js")
  token = localStorage.getItem("jjwxctoken")
  ...
  const catalog = (url) => {
      ...
      $.chapterlist.forEach((chapter) => {
          ...
          name : chapter.chaptername
              ...
              url: chapter.isvip == 0 ? `https://app-cdn.jjwxc.net/androidapi/chapterContent?novelId=${chapter.novelid}&chapterId=${chapter.chapterid}` : `https://android.jjwxc.com/androidapi/androidChapterBatchDownload?novelId=${chapter.novelid}&versionCode=290&token=${token}&chapterIds=${chapter.chapterid}`,
              vip: chapter.isvip == 2
          })
...
if (!url.match(/token/)) return ($.content +"\n作者有话说：\n"+$.sayBody)
else{
    
    content =$.downloadContent[0].content
    if(content){
zzbb = $.downloadContent[0].sayBody
return (decode(content)+"\n\n作者有话说：\n"+zzbb
)}
else return $.downloadContent[0].message
}

  }
function decode(word) {
    let key =CryptoJS.enc.Utf8.parse( "KW8Dvm2N");
  ...
  //个人中心
  const profile = () => {
      let response = GET(`${baseurlapp}/getUserCenter?versionCode=287&token=${token}`)
      let $ = JSON.parse(response)
  ...
  const bookshelf = () => {
      let response = POST("https://app.jjwxc.org/androidapi/incrementFavorite", {
          data: `versionCode=194&token=${token}&classId=&order=0&offset=0&limit=1000`
      })
  ...
  const login = (args) => {
    if(!args) return "token不能为空!"
    localStorage.setItem("jjwxctoken", args)
    }
